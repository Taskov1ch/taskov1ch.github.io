...
const PrevIcon = () => <svg viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"></path></svg>;
const NextIcon = () => <svg viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"></path></svg>;
...
function MiniPlayer({ spotifyLink }) {
  ...
  useEffect(() => {
    const triggerElement = document.getElementById('footer-intersection-trigger');
  ...
  return (
    ...
    >
        ...
        <div className="player-clickable-area" onClick={handleOpenSpotify}>
            <div className="player-cover-container">
                <AnimatePresence mode='wait'>
                    {isLoading ? (
                        <motion.div key="spinner" {...transitionProps} className="spinner-wrapper">
                            <LoadingSpinner />
                        </motion.div>
                    ) : (
                        <motion.img key={currentTrack.id} src={currentTrack.cover} alt="Album Cover" className="player-cover" {...transitionProps} />
                    )}
                </AnimatePresence>
            </div>
            ...
            <div className="player-info">
                <AnimatePresence mode='wait'>
                    <motion.div key={currentTrack.id} {...transitionProps} className="track-info-wrapper">
        ...
        <div className="player-controls">
            <button onClick={(e) => handleControlClick(e, prevTrack)} className="control-btn" aria-label="Previous Track" disabled={isLoading}>
                <PrevIcon />
            </button>
            <button onClick={(e) => handleControlClick(e, togglePlayPause)} className="control-btn play-pause-btn" aria-label={isPlaying ? "Pause" : "Play"} disabled={isLoading}>
                {isPlaying ? <PauseIcon /> : <PlayIcon />}
            </button>
            <button onClick={(e) => handleControlClick(e, nextTrack)} className="control-btn" aria-label="Next Track" disabled={isLoading}>
                <NextIcon />
            </button>
        </div>
    </div>
