...
function Dashboard() {
    ...
    const fetchAnixartStats = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Не удалось получить данные от Anixart. Статус: ${response.status}`
          );
        }
        ...
        setAnixartError(
          e.message ||
            "Произошла неизвестная ошибка при загрузке данных Anixart."
        );
    ...
    const fetchSteamStats = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error || `Steam Stats API error: ${response.status}`
          );
        }
        const data = await response.json();
        setSteamStats(data);
  ...
  useEffect(() => {
    const fetchSteamWishlist = async () => {
      ...
      try {
        ...
        setErrorWishlist(
          e.message || "Ошибка загрузки списка желаемого Steam."
        );
        setSteamWishlist([]);
      } finally {
  ...
  const preparedAnimeStats = useMemo(() => {
    ...
    const counts = {
      watching: anixartStats.watching_count || 0,
      plan: anixartStats.plan_count || 0,
      completed: anixartStats.completed_count || 0,
      hold_on: anixartStats.hold_on_count || 0,
    ...
    return Object.keys(ANIME_STATUS_NAMES_RU)
      .map((statusKey) => ({
        name: ANIME_STATUS_NAMES_RU[statusKey],
        value: counts[statusKey],
        percentage: totalCount > 0 ? (counts[statusKey] / totalCount) * 100 : 0,
        color: ANIME_STATUS_COLORS[statusKey],
        key: statusKey,
      }))
  ...
  const steamAvatarFrameUrl =
    "https://cdn.fastly.steamstatic.com/steamcommunity/public/images/items/2873080/7591f6defdeafebf72d2bfdc75fc7568262557ba.png";
  ...
  return (
    ...
    >
        ...
        {!isLoadingAnixart && !anixartError && anixartStats && (
          <>
            <div className='anixart-profile-header'>
              {anixartStats.avatar && (
                <img
                  src={anixartStats.avatar}
                  alt={anixartStats.login}
                  className='anixart-avatar'
                />
              )}
              <div className='anixart-profile-info'>
                <h3>
                  {anixartStats.login}
                  {anixartStats.badge?.image_url && (
                    <img
                      src={anixartStats.badge.image_url}
                      alt={anixartStats.badge.name}
                      className='anixart-badge'
                      title={anixartStats.badge.name}
                    />
                  )}
                </h3>
                {anixartStats.status && (
                  <p
                    className='anixart-status'
                    dangerouslySetInnerHTML={{
                      __html: anixartStats.status.replace(/\n/g, "<br />"),
                    }}
                  ></p>
                )}
              </div>
              ...
              <ul className='anixart-history-list'>
                {anixartStats.history.slice(0, 10).map((item) => {
                  ...
                  return (
                    <a
                      key={item["@id"] || animeId}
                      href={anixartReleaseUrl}
                      target='_blank'
                      rel='noopener noreferrer'
                      className='anixart-history-item-link'
                    >
                      <li className='anixart-history-item-content'>
                        <img
                          src={
                            item.image ||
                            `https://via.placeholder.com/50x70?text=${item.title_ru?.charAt(0) || "A"}`
                          }
                          alt={item.title_ru || "Обложка аниме"}
                        ...
                        <div className='history-details'>
                          ...
                          <span className='history-episode'>
                            Эпизод: {item.last_view_episode?.name || "N/A"}
                          </span>
                          <span className='history-time'>
                            Просмотрено:{" "}
                            {formatTimestamp(item.last_view_timestamp)}
                          </span>
                        </div>
                      </li>
                    </a>
      ...
      <section className='dashboard-section steam-stats' id='steam-stats'>
        ...
        {!errorSteam && !steamStats && !isLoadingSteam && (
          <p>Данные Steam не загружены.</p>
        )}
        {steamStats && steamStats.profile && (
          <div className='steam-profile-header'>
            <div className='steam-avatar-container'>
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.src = STEAM_AVATAR_FALLBACK_URL;
                }}
              />
              <img
            ...
            <div className='steam-profile-info'>
              <h3>
                <a
                  href={steamStats.profile.profileurl}
                  target='_blank'
                  rel='noopener noreferrer'
                >
              ...
              {steamStats.profile.realname && (
                <p>Имя: {steamStats.profile.realname}</p>
              )}
              {steamStats.profile.loccountrycode && (
                <p>Страна: {steamStats.profile.loccountrycode}</p>
              )}
            </div>
          </div>
        )}
        ...
        {steamStats?.recentlyPlayed && (
          <>
            ...
            {steamStats.recentlyPlayed.length > 0 ? (
                      ...
                      <li className='steam-game-item-content'>
                        ...
                        <div className='steam-game-details'>
                          ...
                          <span className='steam-game-playtime'>
                            За 2 нед.: {formatPlaytime(game.playtime_2weeks)} |
              ...
              <p>
                {steamStats.total_games_played_2weeks > 0
                  ? `Сыграно игр за 2 недели: ${steamStats.total_games_played_2weeks}, но нет детальной статистики.`
                  : "Нет данных о недавно сыгранных играх."}
              </p>
            )}
          </>
        )}
      ...
      >
        ...
        {errorWishlist && (
          <p className='error-message'>Ошибка: {errorWishlist}</p>
        )}
        {wishlistErrorMessage && !isLoadingWishlist && (
          <p className='info-message'>{wishlistErrorMessage}</p>
        )}

        {!isLoadingWishlist &&
          !errorWishlist &&
          !wishlistErrorMessage &&
          steamWishlist.length === 0 && <p>Список желаемого пуст.</p>}
              ...
              return (
                ...
                >
                  <img
                    src={
                      game.header_image ||
                      `https://via.placeholder.com/200x93?text=${game.name || "Game"}`
                    }
                    alt={game.name}
                    className='wishlist-game-image'
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = `https://via.placeholder.com/200x93?text=Error`;
                    }}
                  />
      ...
      <section>Возможно ещё что-то будет здесь...</section>
    </motion.div>
  );
}
